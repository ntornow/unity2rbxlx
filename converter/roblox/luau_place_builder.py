...
def _emit_terrain(b: _LuauBuilder, place: RbxPlace) -> None:
    ...
    if terrain_script:
        # The TerrainGenerator script is already embedded as a server Script.
        # It will run when the place is opened in Studio (play mode).
        # We don't inline it here to avoid block-balance issues with
        # the complex FillBlock Luau code.
        b.line("-- Terrain will be generated by the embedded TerrainGenerator script at runtime")
    else:
