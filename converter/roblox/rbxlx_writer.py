...
def _make_part(parent_xml: ET.Element, part: RbxPart) -> None:
    ...
    if part_class != "Model":
        ...
        # Warn when a visible MeshPart is floored to the 0.05 min: it
        # renders effectively invisible, so make the sizing regression loud.
    ...
    # behave like the published-place chunked builder.
    coll_fid = getattr(part, "collision_fidelity", None)
    if (
        part_class == "MeshPart"
