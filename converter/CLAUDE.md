...
| **`u2r.py publish`** | **Replays `<output>/place_builder_chunks.json`** if cached — preserves the assembled state byte-for-byte. Falls back to a fresh Pipeline rebuild only on cache miss. | When you want to re-publish the assembled state without re-running the converter (e.g., after a transient upload failure), or when the Unity project has been moved/archived. See `u2r.py:208–263`. |
