# Converter TODO — Archive

Historical record of completed work and PR execution logs. Active TODOs live in `TODO.md`.

This file preserves the full session-by-session history: per-priority gap analyses,
PR execution logs (Phase 3 / Phase 4 / Codex review closures), and architectural
decisions. Some entries below are still marked `[ ]` — those are either:
  (a) genuinely deferred follow-ups (now also tracked in `TODO.md`), or
  (b) stale entries that were addressed by a later PR but never re-checked
      (look for "see PR N" notes in the active TODO).

Open work moves to `TODO.md`. Completed work and execution logs stay here.
...
- [x] **Binary animation/controller parsing**: Both `.anim` and `.controller` now route through UnityPy. `.anim` delegates to `_parse_clip_body` after a typetree dump; `.controller` walks the AnimatorController + StateMachine + State + StateTransition + BlendTree typetree graph and translates PPtrs (`m_FileID`/`m_PathID`) into the YAML-style `fileID`/`guid` shape via `_translate_pptrs`, so the same builder drives both formats. External clip GUIDs resolved through the SerializedFile externals list. UNCONVERTED entries still emitted on UnityPy load failure or missing AnimatorController object.
