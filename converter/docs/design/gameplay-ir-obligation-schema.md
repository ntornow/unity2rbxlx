...
| **T-bullet** | bullet spawns but **drops to the ground** (never reaches the player), and even on a forced hit deals no measured damage | **UPDATED 2026-06-15 (Claude+Codex, static+rbxlx; live owed):** the original `parent=nil` framing is STALE — `instantiatePrefab(self.turretBullet, workspace, …)` passes `workspace` explicitly; the clone enters the DataModel and runs Awake/Start (firing works). The real blockers: **(a) #8 projectile physics** — the unanchored bullet uses raw `ApplyImpulse(force=60)` with no stud-scale / anti-gravity / raycast (legacy `bullet_physics_raycast` absent from generic) → arcs into the ground; **(b) damage-target mismatch** — `TurretBullet`→`Player:TakeDamage` mutates `Player.curHealth` only (`Player.luau:332`), NOT `Humanoid.Health` (the F16 fixture's surface), whereas `Mine`→`hum:TakeDamage` hits the real Humanoid. Neither is client/server. Tracked P0 in `TODO.md`; (a) is relation #8 (§3 row 8). Required (with T targeting) for turrets to *kill* |
