...
| 9 | **dynamic-component-ref → driver-domain** `driver(Door.open)=client Animator MB` | a dynamic `GetComponent<Animator>()` (no serialized ref) + C# param-writes (`SetBool`/`SetTrigger`) matched to the clip's `observed_attribute` | resolve the driver → place the generated Anim script in the writer's domain; cross-domain verifier backstops | per-project animation-driver `domain_overrides`; the `animation_routing` serialized-only resolution gap | **CANDIDATE, repro confirmed** (door-scoped; analysis PR #188 2026-06-11; live Studio repro 2026-06-14 — `door:open` resolves `unresolved`→`domain:server` because `Door.cs` uses dynamic `Find("door").GetComponent<Animator>()`, no serialized ref, so the client component's `open` write is never observed by the server Anim script; fix + placement nuances tracked P0 in `TODO.md`; other animation drivers fail for different reasons) |
