...
> **2026-06-11 dual-voice re-investigation (Claude + Codex), Step-1b cold-Studio playtest.** Confirmed Slice T + HudControl still fail, and added the rifle (#3) + door (#4) canary failures + a SECOND turret-bullet bug. None are Step-1b (player-embodiment) regressions — all pre-existing generic-mode gaps where the IR either *abstains* (#1) or doesn't *model/rebind* the edge (#3/#4). File:line evidence in the Step-1b run ledger (`.harness/followups.md` "IR-coverage investigation").
