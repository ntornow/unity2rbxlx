...
Status: **eng-reviewed; gate revised 2026-06-04.** The 9 locked decisions stand; the *hold* is superseded.
Companion: `docs/architecture_critique.md`. Execution sequencing: `docs/design/scene-runtime-and-refactor-execution.md`.
Re-baselined 2026-05-22 (68 upstream commits) and 2026-05-23 (9 more) — see "Drift re-baseline" below.

> **⚠️ 2026-06-04 — numbers stale, gate revised (full re-baseline is Track 0).** This plan migrated from PR #127 (closed; branch 294 commits stale) into the live doc set. Two things changed since the last re-baseline:
> 1. **LOC drift (un-refreshed below):** `pipeline.py` 4917→**6495**, `scene_converter.py` 5542→**5553**, `script_coherence_packs.py` 5051→**5373**, pack count 27→**~30**. The PR-C/PR-D/PR-E LOC targets and the PR-D phase enumeration (now also missing `plan_scene_runtime` + `materialize_and_classify`) must be recomputed in **Track 0** before execution.
> 2. **New modules the original critique predates:** `scene_runtime_topology/` and `contract_verifier.py` landed (PR #148 phases). Both are clean, well-factored modules — **not** split targets — but the file census should acknowledge them.
>
