...
'''
    ...
    # Per-script source markers ``_subphase_inject_scene_runtime`` stamps
    # on the four host-runtime scripts it emits under ``generic`` mode.
    # PR4's R3-P2 fix uses the same markers to distinguish "prior
    # autogen output we may overwrite" from "user-owned script we must
    # preserve". PR6's guard reads them for the second purpose: a
    # user-owned ``SceneRuntime`` (a project that happens to ship a
    # script of that name) MUST NOT count as a valid host emit, or the
    # guard would false-pass and the converted place would still load
    # broken at start.
    _SCENE_RUNTIME_AUTOGEN_MARKERS: ClassVar[dict[str, str]] = {
        "SceneRuntime": "-- scene_runtime: PR4 generic host runtime",
        "SceneRuntimePlan": (
            "-- SceneRuntimePlan (auto-generated by Unity converter; PR4)"
        ),
    ...
    # Names of the four host-runtime scripts above, in the order
    # ``_subphase_inject_scene_runtime`` emits them. PR6's guard walks
    # this tuple looking up each name in ``rbx_place.scripts``; absence
    # OR presence-without-marker counts as "missing autogen emit".
    ...
    def _check_runtime_playability_guard(self) -> None:
        ...
        writes.
        """
        # Codex R1-P2 (PR6 review round 1): clear any prior guard
        # warnings BEFORE the mode gate. ``conversion_context.json``
        # persists ``nonplayable_warnings`` across runs, so a later
        # CLEAN rerun (under either generic or the auto->legacy
        # fallback) would otherwise re-render the stale warning into
        # the new UNCONVERTED.md and lie about the current output.
        # The guard is the single source of truth for the warning --
        ...
        #
        # Codex R1-P1.B (PR6 review round 1): a user-owned script that
        # collides with one of the four reserved names DOES survive the
        # emit subphase (R3-P2 deliberately preserves it to avoid
        # clobbering user content), but it MUST NOT count as a valid
        # host emit -- the engine binding logic only lives in the
        # autogen sources. We narrow the presence check to "script
        # exists AND carries the autogen marker the subphase stamped",
        # so a name-collision project still surfaces the guard.
        scripts_by_name: dict[str, RbxScript] = {}
        for s in self.state.rbx_place.scripts:
            # ``_replace_or_add`` keeps the first match, so do the
            # same here for symmetry; the marker check below tolerates
            # either copy when both exist.
            scripts_by_name.setdefault(s.name, s)
        missing_host_scripts: list[str] = []
        for name in self._SCENE_RUNTIME_HOST_SCRIPTS:
            script = scripts_by_name.get(name)
            if script is None:
                missing_host_scripts.append(name)
                continue
            marker = self._SCENE_RUNTIME_AUTOGEN_MARKERS.get(name, "")
