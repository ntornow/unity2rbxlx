...
return table.concat(allData, "\\n")'''
    ...
    def _subphase_inject_autogen_scripts(self) -> None:
        ...
        #
        # Signal we use: the on-disk presence of ``scripts/HUDController.luau``
        # (or ``FpsClient.luau``). The pre-PR ``inject_fps_scripts`` was the
        # only code path that wrote those file names — finding them means
        # this output dir was generated by that path. Distinguishes a
        # resumed-from-pre-PR run from a fresh post-PR conversion (no
        # pre-existing emit), which ``completed_phases`` cannot do because
        # every fresh phase marks itself complete during the same run.
        if not self.scaffolding and self._fps_artifacts_on_disk():
            log.warning(
