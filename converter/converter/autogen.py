...
# conditional shared-flag funnel fragment. The base carries spawn handling,
# the ``PlayerShoot`` + ``PlayerGetItem`` RemoteEvents, and the shoot/pickup
# handlers — all UNCONDITIONAL. The funnel fragment (the
# ``PlayerSetSharedFlag`` RemoteEvent + its ``OnServerEvent`` listener) is
# spliced in only when ``include_shared_flag_funnel`` is True (the default),
# gated on the ``shared_flag_channels`` topology fact at the inject site.
# The blank line separating ``PlayerGetItem`` from the next block lives at
# the END of ``_GAME_SERVER_HEAD`` so the funnel-omitted case still has a
# clean separator before the visual-hit-feedback section.
