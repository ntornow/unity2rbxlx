...
def generate_scene_runtime_plan_module(
    scene_runtime: dict,
    *,
    ...
    ``scene_namespace`` (R4-P1.1, absorbed in PR5): when set, the
    embedded ``scenes`` table is scoped to that namespace only -- the
    multi-scene pipeline emits one ``.rbxlx`` per scene, and each
    place's plan must carry only its own scene's instances/references
    so the host's ``workspaceFind`` lookup doesn't try to bind
    components for a sister-place's scene. ``None`` keeps the legacy
    behaviour (full ``scenes`` table embedded), used by single-scene
    runs and by tests that don't model the multi-scene pipeline.

    Modules / prefabs / domain_overrides / scriptable_objects are
    project-scoped and NOT filtered -- a prefab's component instances
    can be spawned from any scene, and the modules table is the host's
    domain/container map keyed by ``script_id`` regardless of which
    scene the instance lives in.
    """
    plan_subset: dict = {
    ...
    if scene_namespace is not None:
        scenes_full = plan_subset.get("scenes")
        if isinstance(scenes_full, dict):
            scoped: dict = {}
            if scene_namespace in scenes_full:
                scoped[scene_namespace] = scenes_full[scene_namespace]
            plan_subset["scenes"] = scoped
