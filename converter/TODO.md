...
- [ ] **Alpha-mode transparency for chainlink/grid textures**: Fence chainlink texture renders opaque (orange rectangle) instead of transparent-where-no-wire. The SurfaceAppearance.AlphaMode is `Overlay` but should be `Transparency` when the texture has alpha. Needs: detect alpha channel in the source texture (PSD/PNG) and set `alpha_mode="Transparency"` in the material mapper.
