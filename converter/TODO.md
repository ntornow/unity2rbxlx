...
- [ ] **Object pooling runtime shim**: Unity ObjectPool<T>/pool.Free()/pool.GetNew() patterns transpile to hollow stubs. Fix: inject a lightweight Luau pooling module that caches and recycles Part/Model instances via ReplicatedStorage templates.
- [ ] **TODO placeholders in transpiled scripts**: 20+ scripts have `-- TODO: implement X` placeholders that silently fail at runtime. Fix: preflight scan for TODO markers and flag scripts as requiring manual review, or generate error-logging fallbacks.
- [ ] **OnCollisionStay/OnCollisionExit**: Referenced in some scripts but the runtime only handles Touched (OnCollisionEnter). Stay/Exit need TouchEnded and per-frame contact tracking.
