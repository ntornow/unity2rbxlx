...
3. `convert_animations` — `converter/animation_converter.py` converts `.anim` / `.controller` files into TweenService Luau scripts (transform animation) or Motor6D bone chains (skeletal). 5 runtime modules under `runtime/` are auto-injected (animator, nav mesh, event system, physics bridge, cinemachine).
