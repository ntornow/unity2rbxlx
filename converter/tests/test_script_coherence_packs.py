...
# ---------------------------------------------------------------------------
# Return-aware block insertion (Bug D). Generic scene-runtime modules end
# in ``return <Class>``; a coherence pack that appends a ``do...end`` block
# AFTER that return produces illegal Luau ("Expected <eof>, got 'do'").
# Packs must splice their block BEFORE the terminal return.
# ---------------------------------------------------------------------------

class TestInsertBeforeTerminalReturn:
    """Unit coverage for the shared ``_insert_before_terminal_return``
    helper and the door-tween pack that regressed in SimpleFPS generic."""

    def test_helper_inserts_before_return(self) -> None:
        src = "local Door = {}\n\nreturn Door\n"
        block = "do\n    print('x')\nend"
        out = packs_module._insert_before_terminal_return(src, block)
        # Block lands before the return; the module still ENDS with it.
        assert out.index("do\n") < out.index("return Door")
        assert out.rstrip().endswith("return Door")

    def test_helper_appends_when_no_terminal_return(self) -> None:
        # Legacy top-level Script: no terminal return -> plain append.
        src = "print('legacy script body')\n"
        block = "do\n    print('x')\nend"
        out = packs_module._insert_before_terminal_return(src, block)
        assert out.rstrip().endswith("end")

    def _generic_door_module(self) -> RbxScript:
        # A generic ModuleScript shaped like the SimpleFPS Door.luau:
        # flips the ``open`` attribute on a sibling ``door`` mesh, ends
        # with ``return Door``. This trips the door_tween_open detector.
        return RbxScript(
            name="Door",
            source=(
                "local Door = {}\n"
    ...
    def test_door_tween_pack_keeps_return_terminal(self) -> None:
        s = self._generic_door_module()
        fixes = packs_module._inject_door_tween([s])
        assert fixes == 1
        # The injected tween block must land BEFORE the terminal return.
        assert "_AutoFpsDoorTweenInjected" in s.source
        assert s.source.index("_AutoFpsDoorTweenInjected") < s.source.index(
            "return Door"
        )
        # Module still ENDS with the return -- no statement after it.
        assert s.source.rstrip().endswith("return Door")
        # The pre-fix symptom was a ``do`` block after the return.
        after_return = s.source.rstrip()
        assert after_return[after_return.rindex("return Door"):].strip() == (
            "return Door"
        )

    def test_door_tween_pack_idempotent(self) -> None:
        s = self._generic_door_module()
        assert packs_module._inject_door_tween([s]) == 1
        assert packs_module._inject_door_tween([s]) == 0
        # Exactly one return Door, still terminal.
        assert s.source.count("return Door") == 1
        assert s.source.rstrip().endswith("return Door")
