...
class TestResumeVsRunThroughParity:
    ...
    def test_essential_phases_is_single_source_of_truth(self):
        ...
        assert Pipeline.ESSENTIAL_PHASES == frozenset({
            ...
            # Addressables Unit 1 (D8): ``plan_scene_runtime`` recomputes
            # the addressables block + resolved ``template_name`` map fresh
            # on every resume so a ``--phase=write_output`` resume never
            # pairs a fresh prefab_library with a stale persisted map.
            "plan_scene_runtime",
